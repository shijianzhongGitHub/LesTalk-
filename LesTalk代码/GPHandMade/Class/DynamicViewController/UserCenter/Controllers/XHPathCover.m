...
{
    NSLog(@"背景图片类型==== %@",[backgroundImageUrlString class]);
    if([backgroundImageUrlString isKindOfClass:[NSString class]])
    {
        if([backgroundImageUrlString isEqualToString:@"http://share.du-ang.cn/404_774-978.jpg_thumb1"])
        {
            _bannerImageView.image =[UIImage imageNamed:@"userTeamBackG.jpg"];
        }
        else
        {
            NSURL *imgURL = [NSURL URLWithString:backgroundImageUrlString];
            [_bannerImageView sd_setImageWithURL:imgURL placeholderImage:[UIImage imageNamed:@"userTeamBackG.jpg"]];
        }

    }
    else if([backgroundImageUrlString isKindOfClass:[UIImage class]])
    {
        _bannerImageView.image = (UIImage *)backgroundImageUrlString;
    }
    else
    {
        _bannerImageView.image = [UIImage imageNamed:@"userTeamBackG.jpg"];
    ...
    [_bannerImageView setContentScaleFactor:[[UIScreen mainScreen] scale]];
}
// avatar
...
{
    self.offsetY = scrollView.contentOffset.y-20.0;
...
{
    CGFloat fixAdaptorPadding = -20;
    if ([[[UIDevice currentDevice] systemVersion] integerValue] >= 7.0) {
        fixAdaptorPadding = -20;////////////////
    }
    y += fixAdaptorPadding;
...
- (void)_setup
{
    self.parallaxHeight = 240;
    self.isLightEffect = YES;
    self.lightEffectPadding = 0;
    self.lightEffectAlpha = 1;
    
    _bannerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
    _bannerView.clipsToBounds = YES;
    _bannerImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0,0, CGRectGetWidth(_bannerView.frame), CGRectGetHeight(_bannerView.frame))];
